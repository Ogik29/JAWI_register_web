                ...
                <form id="editForm" method="POST" action="{{ route('player.update', $player->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="athlete-card">
                        <div class="athlete-header">
                            <h5 class="mb-0"><i class="fas fa-user-ninja me-2"></i>Data Atlet</h5>
                        </div>
                        <div class="p-4">
                             @if ($player->contingent->status == 2)
                                <div class="alert alert-warning" role="alert">
                                    <i class="fas fa-exclamation-triangle me-2"></i>
                                    Kontingen induk peserta ini sedang ditolak. Menyimpan perubahan akan mengubah status kontingen menjadi 'Menunggu Verifikasi'.
                                </div>
                            @endif
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" name="nik" value="{{ old('nik', $player->nik) }}" pattern="[0-9]{16}" maxlength="16" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">No Telepon</label>
                                    <input type="tel" class="form-control" name="no_telp" value="{{ old('no_telp', $player->no_telp) }}" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control" name="tgl_lahir" value="{{ old('tgl_lahir', $player->tgl_lahir) }}" required>
                                </div>
                                
                                <div class="col-md-12 mb-3">
                                    <label class="form-label fw-bold">Kelas Pertandingan</label>
                                    <select class="form-select" name="kelas_pertandingan_id" required>
                                        <option value="">Pilih Kelas Pertandingan</option>
                                        @foreach ($player->contingent->event->kelasPertandingan as $kelas)
                                            <option value="{{ $kelas->id }}" {{ old('kelas_pertandingan_id', $player->kelas_pertandingan_id) == $kelas->id ? 'selected' : '' }}>
                                                {{ $kelas->nama_kelas }} ({{ $kelas->kategoriPertandingan->nama_kategori }} - {{ $kelas->jenisPertandingan->nama_jenis }} - {{ $kelas->rentang_usia }} - {{ $kelas->gender }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    @if ($player->foto_ktp)
                                        <div class="file-info">
                                            ...
                                            <a href="{{ Storage::url($player->foto_ktp) }}" target="_blank">Lihat file saat ini</a>
                                        </div>
                                    @endif
                                    <div class="upload-area mt-2">
                                        <p class="mb-2">Ganti file KK/KTP (opsional)</p>
                                        <input type="file" class="form-control" name="foto_ktp" accept=".jpg,.jpeg,.png,.pdf">
                                    </div>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label fw-bold">Upload Foto Diri</label>
                                     @if ($player->foto_diri)
                                        <div class="file-info">
                                            <i class="fas fa-check-circle text-success me-2"></i>
                                            <a href="{{ Storage::url($player->foto_diri) }}" target="_blank">Lihat foto saat ini</a>
                                        </div>
                                    @endif
                                    <div class="upload-area mt-2">
                                        <p class="mb-2">Ganti foto diri (opsional)</p>
                                        <input type="file" class="form-control" name="foto_diri" accept=".jpg,.jpeg,.png">
                                    </div>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label fw-bold">Upload Persetujuan Ortu</label>
                                    @if ($player->foto_persetujuan_ortu)
                                        <div class="file-info">
                                            <i class="fas fa-check-circle text-success me-2"></i>
                                            <a href="{{ Storage::url($player->foto_persetujuan_ortu) }}" target="_blank">Lihat file saat ini</a>
                                        </div>
                                    @endif
                                    <div class="upload-area mt-2">
                                        <p class="mb-2">Ganti file persetujuan (opsional)</p>
                                        <input type="file" class="form-control" name="foto_persetujuan_ortu" accept=".jpg,.jpeg,.png,.pdf">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="text-center mt-4 d-flex justify-content-end">
                        <a href="{{ route('history') }}" class="btn btn-secondary btn-lg me-3">
