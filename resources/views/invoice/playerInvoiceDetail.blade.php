...
<head>
    ...
    <title>Invoice - {{ $contingent->name }} - #INV-{{ $invoice->id }}</title>
...
<body class="bg-gray-50 min-h-screen py-8">
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden invoice-card">
            ...
            <div class="text-white p-8 bg-gradient-to-r from-emerald-800 to-teal-800">
                <div class="flex justify-between items-start flex-wrap gap-4">
                    <div>
                        ...
                        <h1 class="text-3xl font-bold tracking-tight mb-2">INVOICE PEMBAYARAN ATLET</h1>
                    ...
                    <div class="text-left md:text-right">
                        <div class="bg-white bg-opacity-10 text-white px-4 py-2 rounded-lg font-bold text-lg border border-white border-opacity-20 inline-block">#INV-{{ $invoice->id }}</div>
                        <p class="text-emerald-100 text-sm mt-3">Tanggal: {{ \Carbon\Carbon::parse($invoice->date)->translatedFormat('d F Y') }}</p>
            ...
            <!-- Athlete Table -->
            <div class="p-8">
                <h3 class="font-semibold text-gray-800 mb-4 uppercase tracking-wider text-xs">Detail Rincian Pendaftaran Peserta:</h3>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @forelse ($invoiceItems as $item)
                                <tr>
                                    <td class="py-4 px-2">
                                        <div class="font-semibold text-gray-900">{{ $item['nama_kelas'] }} ({{ $item['gender'] }})</div>
                                    ...
                                    <td class="text-right py-4 px-2 font-semibold text-emerald-700">Rp {{ number_format($item['harga_per_pendaftaran'], 0, ',', '.') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4 text-gray-500">Tidak ada rincian data pendaftaran.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="mt-8 flex justify-end">
                    <div class="w-full max-w-sm">
                        <div class="flex justify-between py-2 border-b border-gray-200 text-sm">
                            ...
                            <span class="font-medium text-gray-800">Rp {{ number_format($invoice->total_price, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between py-3 border-b-2 border-gray-300">
                            ...
                            <span class="text-lg font-bold text-emerald-800">Rp {{ number_format($invoice->total_price, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="w-full md:w-1/2 text-center">
                        @if ($invoice->foto_invoice)
                            <div class="p-3 bg-white border rounded-lg shadow-sm inline-block">
                                <img src="{{ Storage::url($invoice->foto_invoice) }}" class="max-h-64 rounded object-contain mx-auto" alt="Bukti Transfer">
                            </div>
                            <div class="mt-3 no-print">
                                <a href="{{ Storage::url($invoice->foto_invoice) }}" target="_blank" class="inline-flex items-center text-xs text-gray-600 hover:text-gray-900 border px-3 py-1.5 rounded bg-white shadow-sm font-medium">
                    ...
                    <div class="w-full md:w-1/2 text-gray-600 text-sm">
                        <div class="bg-white p-4 border rounded-lg shadow-sm">
                            ...
                            <ul class="list-disc pl-5 space-y-1.5 text-xs text-gray-600">
                                <li>Pembayaran telah berhasil diverifikasi oleh administrator Jawara Indonesia.</li>
                                <li>Status kepesertaan atlet di atas resmi terdaftar pada event ini.</li>
                                <li>Kartu peserta/atlet dapat dicetak melalui halaman riwayat pendaftaran kontingen.</li>
            ...
            <div class="bg-gray-950 text-neutral-400 p-6 text-center text-xs">
                <p>Terima kasih atas pembayaran Anda. Pendaftaran Anda telah kami terima.</p>
                <p class="mt-1 text-neutral-500">Invoice pembayaran otomatis #INV-{{ $invoice->id }} • JAWI Jawara Indonesia</p>
            </div>
        </div>
    </div>
</body>
