...
<head>
    ...
    <style>
        body { background: linear-gradient(135deg, #ffffffff 0%, #dfdfdfff 100%); min-height: 100vh; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding-bottom: 150px; /* Ruang untuk footer */ }
        ...
        .total-price-footer {
            background: #ffffff;
            border-top: 3px solid #c50000ff;
            padding: 20px 40px;
            position: fixed; /* Mengambang di bawah */
            bottom: 0;
            left: 0;
            right: 0;
            z-index: 1000;
...
<body>
            ...
            <div class="p-4">
                ...
                <form id="registrationForm">
                    ...
                    <div class="text-center my-4">
                        <button type="button" class="btn btn-custom" id="addAthleteBtn"><i class="fas fa-plus me-2"></i>Tambah Pendaftaran Kelas</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="total-price-footer d-flex justify-content-end align-items-center">
        <div class="me-4 text-end">
            <h5 class="mb-0">Total Biaya Pendaftaran:</h5>
            <h2 class="fw-bold text-danger" id="totalPriceDisplay">Rp 0</h2>
        </div>
        <button type="submit" form="registrationForm" class="btn btn-custom btn-lg" id="submitBtn">
            <i class="fas fa-paper-plane me-2"></i>Daftar & Bayar
        </button>
    </div>
...
<template id="athleteCardTemplate">
    <div class="athlete-card" data-card-id="__ID__">
        ...
        <div class="p-4">
            ...
            <div class="row">
                <div class="col-12 mb-3"><label class="form-label fw-bold">4. Pilih Kelas Pertandingan</label><select class="form-select" name="kelas_pertandingan_id" required><option value="">Lengkapi filter di atas</option></select></div>
            </div>
            <hr>
...
<template id="subAthleteFormTemplate">
    <div class="sub-athlete-form">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3"><label class="form-label">Jenis Kelamin</label><select class="form-select" name="jenisKelamin" required><option value="" selected disabled>Pilih...</option><option value="laki-laki">Laki-laki</option><option value="perempuan">Perempuan</option></select></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const updateAvailableClasses = (card) => {
        ...
        const filteredClasses = KELAS_PERTANDINGAN_DATA.filter(k => k.rentang_usia_id == selectedRentang && k.kategori_pertandingan_id == selectedKategori && k.jenis_pertandingan_id == selectedJenis);
    ...
    const generateSubAthleteForms = (card, playerCount) => {
        ...
        if (e.target.name === 'kelas_pertandingan_id') {
            ...
            if (kelasId) {
                ...
                if (selectedClassData) {
                    generateSubAthleteForms(card, selectedClassData.jumlah_pemain || 1);
                }
            } else {
                card.querySelector('.sub-athlete-container').innerHTML = '';
            }
    ...
    registrationForm.addEventListener('submit', function(e) {
        ...
        document.querySelectorAll('.athlete-card').forEach((card, cardIndex) => {
            ...
            card.querySelectorAll('.sub-athlete-form').forEach((subForm, subIndex) => {
                formData.append(`registrations[${cardIndex}][players][${subIndex}][namaLengkap]`, subForm.querySelector(`input[name="namaLengkap_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][nik]`, subForm.querySelector(`input[name="nik_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][noTelepon]`, subForm.querySelector(`input[name="noTelepon_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][email]`, subForm.querySelector(`input[name="email_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][jenisKelamin]`, subForm.querySelector(`select[name="jenisKelamin_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][tanggalLahir]`, subForm.querySelector(`input[name="tanggalLahir_${subIndex}"]`).value);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][uploadKTP]`, subForm.querySelector(`input[name="uploadKTP_${subIndex}"]`).files[0]);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][uploadFoto]`, subForm.querySelector(`input[name="uploadFoto_${subIndex}"]`).files[0]);
                formData.append(`registrations[${cardIndex}][players][${subIndex}][uploadPersetujuan]`, subForm.querySelector(`input[name="uploadPersetujuan_${subIndex}"]`).files[0]);
            });
        });
        if (hasError) {
